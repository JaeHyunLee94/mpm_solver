...
#define GLEW_STATIC
  ...
  // glewExperimental is required for OpenGL Core Profile (mandatory on macOS).
  // Without it glewInit() fails and all GL function pointers stay null → SIGSEGV.
  glewExperimental = GL_TRUE;
  if (glewInit() != GLEW_OK) {
    fprintf(stderr, "Failed to initialize OpenGL loader!\n");
    glfwDestroyWindow(m_builder_window);
    m_builder_window = nullptr;
    glfwTerminate();
    return *this;
  }
